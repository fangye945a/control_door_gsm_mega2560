...
void GSM_MQTT::OnConnect(void)                     //连接请求成功后
{
    ...
    if( restart_times != 0 ) //如果重启次数补位0 则清零
    {
        restart_times=0;  //重连次数清零
        int eeAddress = sizeof(warning_keep)+sizeof(tmp_card_Enable)+sizeof(locked_keep);
        EEPROM.put(eeAddress,restart_times); //存入EEPROM
    }
    
    if( SD.exists(filetype[1]) )   //如果刷卡记录文件存在 则打开文件
    {
        record_File = SD.open(filetype[1]);
#ifdef DE_BUG
        if(record_File)
            mySerial.println("Open record_file success");
        else
             mySerial.println("Open record_file fail");
#endif 
    }
}
  ...
  if (pingFlag == true)
  {
    ...
    if ((currentMillis - _PingPrevMillis ) >= _KeepAliveTimeOut * 1000)//如果时间大于心跳周期则发送ping包
    {
      ...
      if(_ping_return_flag > 1)//如果发送2次心跳后还没有收到心跳回复则认为设备已离线
      {
        ...
        restart_times++;  //重连次数增加
        int eeAddress = sizeof(warning_keep)+sizeof(tmp_card_Enable)+sizeof(locked_keep);
        EEPROM.put(eeAddress,restart_times);
        
        calibration_seconds = calibration_seconds + ( (millis()/1000) - system_seconds);//记录重启前时间
        system_seconds = millis()/1000;  //更新对应系统时间
        eeAddress +=  sizeof(restart_times);
...
//#endif
  ...
  else
  {
      ...
      _ping();
      if(record_File && MQTT_Flag )  //如果文件存在，且已经打开
      {
#ifdef DE_BUG
          mySerial.println("put_record...");
#endif
          put_record(record_File);  //上报一条刷卡记录
      }
  }
  wdt_reset(); //喂狗操作
}

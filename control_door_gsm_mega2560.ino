...
#define RECONNECT_TIMES_THRESHOLD  3    //进入重连等待的失败次数      
...
unsigned char restart_times = 0; //MQTT重连尝试次数
...
void Param_init()
{
    ...
    eeAddress += sizeof(restart_times);   //系统起始时间
    ...
    mySerial.print("restart_times =");
    mySerial.println(restart_times);
...
void Parse_Json_message_Task(char *json)
{
    ...
    switch(optcode)
    {
        ...
        case 7:                         //校准时间
        {
            ...
            mySerial.println(calibration_seconds);
#endif
            int eeAddress = sizeof(warning_keep)+sizeof(tmp_card_Enable)+sizeof(locked_keep)+sizeof(restart_times);
...
void IO_init()
{
    ...
    pinMode(rfid_reset,OUTPUT);  //GSM模块复位初始化
...
void TimeEvent()
{
    ...
    if( (current_seconds/1000) - system_seconds >= 60 ) //每分钟进行校时
    {
        ...
        int eeAddress = sizeof(locked_keep) + sizeof(tmp_card_Enable) + sizeof(warning_keep) + sizeof(restart_times);
        EEPROM.put(eeAddress,calibration_seconds);  //存入EEPROM
#ifdef DE_BUG
        mySerial.print("nowtime =");
        mySerial.println(calibration_seconds);
...
{
    current_seconds = millis();
    if( current_seconds > RECONNECT_WAIT_TIME 
       || restart_times < RECONNECT_TIMES_THRESHOLD )
    {
